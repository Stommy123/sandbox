...
const ALL_DRINKS_URL = 'https://www.thecocktaildb.com/api/json/v2/REPLACE_WITH_KEY/filter.php?a=Alcoholic';

const ID_LOOKUP_BASE_URL = 'https://www.thecocktaildb.com/api/json/v2/REPLACE_WITH_KEY/lookup.php?i=';
...
const seedDatabase = async () => {
  ...
  for (const requestChunk of chunk(resolveDrinkRequests, 50)) {
    await Promise.all(requestChunk);
  }

  fs.writeFile(__dirname + '/./bad-data.json', JSON.stringify(brokenDrinks), err => {
    if (err) {
      console.log('err', err);
    } else {
  ...
  fs.writeFile(__dirname + '/./data.json', JSON.stringify(resolvedDrinks), err => {
    if (err) {
      console.log('err', err);
    } else {
